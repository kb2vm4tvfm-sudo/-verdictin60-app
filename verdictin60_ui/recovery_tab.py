"""Recovery tab construction, moved from app.py (Phase 8 refactor, no behavior change).

``build_recovery_tab(app, parent)`` builds the widgets exactly as the former
``App._build_recovery_tab`` method did, with all callbacks still pointing
back to the running ``App`` instance passed in as ``app``.
"""
import tkinter as tk

from verdictin60_ui import BG, WHITE, CRIMSON, CRIMSON_HOT, LIGHT_GRAY, _make_lbtn
